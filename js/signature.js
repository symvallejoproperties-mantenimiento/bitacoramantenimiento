export function signature(canvas){
  const c=canvas.getContext('2d');let drawing=false,last,hasInk=false;
  ...
  const move=e=>{if(!drawing)return;e.preventDefault();const p=pos(e);c.strokeStyle='#173c3a';c.lineWidth=2.5;c.lineCap='round';c.beginPath();c.moveTo(last.x,last.y);c.lineTo(p.x,p.y);c.stroke();last=p;hasInk=true};
